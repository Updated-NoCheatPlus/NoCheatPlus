...
public class PhysicsEnvelope {
   ...
   /* public static boolean isExtremeMoveLegit() {
    ...
    /**
     * Estimate how many client-side "hidden" moves were performed before the current one was sent to the server.
     * Current use of this method is for fixing players breaking blocks beneath them<br>
     * <p>
     * The cumulative distance of hidden moves cannot exceed sqrt(1/256) (See: {@link Magic#CraftBukkit_minMoveDist})
     * </p>
     *
     * @param data 
     * @return The estimated number of hidden moves.
     */
    public static int estimateHiddenClientMoves(final MovingData data) {
        double cumulativeDist = 0.0;
        int hiddenCount = 1;
        
        for (int i = 0; i <= data.playerMoves.getNumberOfPastMoves(); i++) {
            final PlayerMoveData pastMove = data.playerMoves.getPastMove(i);
            if (pastMove == null || !pastMove.toIsValid) break;
            final double dist = pastMove.hDistance + pastMove.yDistance;
            cumulativeDist += dist;
            // Stop once total hidden distance exceeds the limit
            if (cumulativeDist > Magic.CraftBukkit_minMoveDist) {
                break;
            }
            hiddenCount++;
        }
    ...
    /**
     * Test if the player made contact with ground within the last pastMoves.
     * 
     * @param data
     * @param pastMoves
     *            Number of past moves to check, at most the number of available past moves.
     * @return
     */
    public static boolean madeContactWithGroundWithin(final MovingData data, int pastMoves) {
        pastMoves = Math.min(pastMoves, data.playerMoves.getNumberOfPastMoves());
        for (int i = 0; i <= pastMoves; i++) {
            final PlayerMoveData pastMove = data.playerMoves.getPastMove(i);
            if (pastMove.to.onGround || pastMove.from.onGround) {
                return true;
            }
        }
        return false;
